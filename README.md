<!-- ═══════════════════════════ HEADER ═══════════════════════════ -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:13547a,100:00c8ff&height=220&section=header&text=YanIanz&fontSize=80&fontColor=ffffff&animation=fadeIn&fontAlignY=34&desc=Crafting%20worlds%20in%20Minecraft,%20one%20plugin%20at%20a%20time&descAlignY=54&descSize=18" width="100%" />
</p>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&duration=2600&pause=800&color=00C8FF&center=true&vCenter=true&width=650&lines=Minecraft+Plugin+Developer+%F0%9F%8E%AE;Spigot+%2F+Paper+Craftsman+%E2%9A%92%EF%B8%8F;DevOps+%26+Cloud+Enthusiast+%E2%98%81%EF%B8%8F;Bugs+don't+stand+a+chance+%F0%9F%90%9B%E2%9A%94%EF%B8%8F" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=YanIanZ&label=PROFILE+VIEWS&color=00c8ff&style=for-the-badge" alt="views" />
  <a href="https://github.com/YanIanZ?tab=followers">
    <img src="https://img.shields.io/github/followers/YanIanZ?label=FOLLOWERS&style=for-the-badge&color=00c8ff&labelColor=0f2027" alt="followers" />
  </a>
  <img src="https://img.shields.io/badge/STATUS-ONLINE-00c8ff?style=for-the-badge&labelColor=0f2027" alt="status" />
</p>

<img src="https://raw.githubusercontent.com/BraveHeart-hue/BraveHeart-hue/main/assets/line-neon.gif" width="100%" onerror="this.style.display='none'" />

---

<!-- ═══════════════════════════ ABOUT ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="32"> About Me

<img align="right" alt="coding" width="330" src="https://raw.githubusercontent.com/devSouvik/devSouvik/master/gif3.gif" />

```yaml
name:      YanIanz
role:      Minecraft Plugin Developer
location:  Indonesia 🇮🇩
focus:     Spigot / Paper plugins & server infrastructure
learning:  Advanced Java optimization & plugin architecture
seeking:   Help scaling Minecraft server infrastructure
open_to:   Minecraft open-source collaborations
fuel:      ["☕ coffee", "🎧 lo-fi playlist", "🌙 3AM commits"]
```

- 🔭 &nbsp;Currently building **custom Spigot / Paper plugins** and tuning server configs
- 🌱 &nbsp;Deep-diving into **JVM tuning, async patterns & clean plugin architecture**
- 👯 &nbsp;Looking to collaborate on **Minecraft open-source projects**
- 💬 &nbsp;Ask me about **plugins, server setup, DevOps & cloud hosting**
- ⚡ &nbsp;Fun fact: **I code better with music on — playlist never stops, bugs don't stand a chance!**

<br clear="right"/>

---

<!-- ═══════════════════════════ STACK ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/WFZvB7VIXBgiz3oDXE/giphy.gif" width="32"> Tech Arsenal

<details open>
<summary><b>🎮 &nbsp;Minecraft Development</b></summary>
<br>
<p align="center">
  <img src="https://skillicons.dev/icons?i=java,gradle,maven,idea&theme=dark" />
  <br><br>
  <img src="https://img.shields.io/badge/Spigot-ED8106?style=for-the-badge&logo=minecraft&logoColor=white" />
  <img src="https://img.shields.io/badge/PaperMC-000000?style=for-the-badge&logo=minecraft&logoColor=white" />
  <img src="https://img.shields.io/badge/Bungee%2FVelocity-1E88E5?style=for-the-badge&logo=minecraft&logoColor=white" />
</p>
</details>

<details>
<summary><b>☁️ &nbsp;Cloud & Hosting</b></summary>
<br>
<p align="center">
  <img src="https://skillicons.dev/icons?i=aws,cloudflare,netlify,vercel&theme=dark" />
  <br><br>
  <img src="https://img.shields.io/badge/Vultr-007BFC?style=for-the-badge&logo=vultr&logoColor=white" />
</p>
</details>

<details>
<summary><b>🛠️ &nbsp;Server & Infrastructure</b></summary>
<br>
<p align="center">
  <img src="https://skillicons.dev/icons?i=nginx,linux,docker,bash,git,github&theme=dark" />
</p>
</details>

<details>
<summary><b>🗄️ &nbsp;Databases</b></summary>
<br>
<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,sqlite,mongodb,redis&theme=dark" />
</p>
</details>

---

<!-- ═══════════════════════════ PROJECTS ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/dWesBcTLavkZuG35MI/giphy.gif" width="32"> Featured Work

<p align="center">
  <a href="https://sourby.my.id">
    <img src="https://img.shields.io/badge/🚀_SOURBY-Plugin_Infrastructure_for_the_Minecraft_Economy-00c8ff?style=for-the-badge&labelColor=0f2027" />
  </a>
</p>

> **Sourby** — plugin distribution, licensing & operational control for server owners and plugin devs.
> Pin your best repos below and they'll show up as cards here. 👇

<p align="center">
  <a href="https://github.com/YanIanZ">
    <img src="https://img.shields.io/badge/EXPLORE_ALL_REPOS-→-0f2027?style=for-the-badge&logo=github&logoColor=00c8ff" />
  </a>
</p>

---

<!-- ═══════════════════════════ GAME ZONE ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/LMt9638dO8dftAjtco/giphy.gif" width="32"> Game Zone — Yes, You Can Actually Play

<p align="center">
  <img src="https://img.shields.io/badge/PRESS_START-00c8ff?style=for-the-badge&labelColor=0f2027" />
</p>

### 🗺️ &nbsp;The Deep Dark Descent — a text adventure

A choose-your-own-adventure dungeon crawl that runs entirely inside GitHub. No install. No JS. Just click.

<p align="center">
  <a href="./game/start.md">
    <img src="https://img.shields.io/badge/▶_START_THE_ADVENTURE-13547a?style=for-the-badge&logo=minecraft&logoColor=00c8ff" height="42" />
  </a>
</p>

> ⛏️ You wake up at bedrock level with 3 hearts, a wooden pickaxe and a bad feeling.
> Can you make it out with the diamonds — or will you hear that hiss?

<br>

### 🐍 &nbsp;Snake Eating My Contributions

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/YanIanZ/YanIanZ/output/snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/YanIanZ/YanIanZ/output/snake.svg" />
    <img alt="snake animation" src="https://raw.githubusercontent.com/YanIanZ/YanIanZ/output/snake.svg" />
  </picture>
</p>

<br>

### 🧠 &nbsp;Guess the Block — click to reveal

<details>
<summary><b>1.</b> &nbsp;I'm found deep underground, I glow blue, and redstone lovers fight over me. What am I?</summary>

> **Lapis Lazuli Ore** — the enchanting table's best friend. 💙
</details>

<details>
<summary><b>2.</b> &nbsp;I'm silent, I'm green, and I'll ruin your build in one second flat.</summary>

> **Creeper.** *Aw man.* 💥
</details>

<details>
<summary><b>3.</b> &nbsp;In code: which event fires when a player joins a Spigot server?</summary>

```java
@EventHandler
public void onJoin(PlayerJoinEvent event) {
    event.setJoinMessage("§a" + event.getPlayer().getName() + " joined the world!");
}
```
> **`PlayerJoinEvent`** — and please, don't do heavy I/O on the main thread here. 🧵
</details>

<details>
<summary><b>4.</b> &nbsp;I have 15 levels of light, I can't be crafted, and mobs hate me most.</summary>

> **Torch** — cheapest anti-mob defense in the game. 🔥
</details>

---

<!-- ═══════════════════════════ STATS ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/ZDOsRhkQfPFxu2vpZ0/giphy.gif" width="32"> GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.shion.dev/api?username=YanIanZ&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&bg_color=0D1117&title_color=00c8ff&icon_color=00c8ff" height="180" alt="stats" />
  <img src="https://github-readme-stats.shion.dev/api/top-langs/?username=YanIanZ&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&layout=compact&bg_color=0D1117&title_color=00c8ff" height="180" alt="top langs" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=YanIanZ&theme=tokyonight&hide_border=true&background=0D1117&ring=00c8ff&fire=00c8ff&currStreakLabel=00c8ff" alt="streak" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=YanIanZ&bg_color=0D1117&color=00c8ff&line=00c8ff&point=ffffff&area=true&hide_border=true" width="98%" alt="activity graph" />
</p>

---

<!-- ═══════════════════════════ TROPHIES ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/QWvra259h4LCvdJnxP/giphy.gif" width="32"> Achievements Unlocked

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=YanIanZ&theme=onedark&no-frame=true&no-bg=true&column=7&margin-w=8&margin-h=8" alt="trophies" />
</p>

---

<!-- ═══════════════════════════ QUOTE ═══════════════════════════ -->
<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="dev quote" />
</p>

---

<!-- ═══════════════════════════ CONNECT ═══════════════════════════ -->
## <img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="28"> Let's Connect

<p align="center">
  <a href="https://sourby.my.id"><img src="https://img.shields.io/badge/Website-0f2027?style=for-the-badge&logo=googlechrome&logoColor=00c8ff" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="#"><img src="https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white" /></a>
</p>

<br>

<p align="center">
  <i>🎧 Currently vibing to music while writing the next plugin...</i>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00c8ff,50:13547a,100:0f2027&height=140&section=footer" width="100%" />
</p>
